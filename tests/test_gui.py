...
@pytest.fixture
def gui_window(app):  # noqa: ARG001
    """Fixture para criar uma instância de TextaGuiWindow."""
    return TextaGuiWindow()
...
def test_gui_button_click(gui_window, mocker):
    ...
    gui_window._on_button_clicked()  # noqa: SLF001
...
def test_gui_reset_state_success(gui_window, mocker):
    ...
    # Configurar para um estado inicial de processamento e tornar a janela visível
    gui_window._set_processing_state(is_processing=True)  # noqa: SLF001
    gui_window.show()  # Mostrar a janela para que set_status seja chamado

    # Chamar reset_state com sucesso=True
    gui_window.reset_state(True)  # noqa: FBT003
...
def test_gui_reset_state_failure(gui_window, mocker):
    ...
    gui_window.reset_state(False)  # noqa: FBT003
...
def test_workflow_manager_update_gui_signals(mocker):  # noqa: ARG001
    """
    ...
    with (
        ...
        # Executar o fluxo
        manager._lock = MagicMock()  # Evitar bloqueio real  # noqa: SLF001
...
def test_workflow_manager_error_handling_signals(mocker):  # noqa: ARG001
    """
    ...
    # Simular um erro na captura (usando tipo de exceção capturado pelo código)
    with (
        patch("src.main.capture_selected_text", side_effect=OSError("Erro simulado")),
        patch("time.sleep"),
        patch("pyperclip.paste", return_value=""),
        patch("pyperclip.copy"),
        ...
        call_args, call_kwargs = manager.update_status.emit.call_args
        assert "Erro" in call_args[0]  # Primeiro argumento posicional (mensagem)
        assert call_kwargs.get("is_error") is True  # Argumento nomeado is_error=True


def test_app_icon_setup(gui_window, mocker):
    """Testa a configuração do ícone do aplicativo com logo existente."""
    # Mock para Path.exists para controlar o resultado
    ...
    mocker.patch(
        "src.gui.QApplication.instance", return_value=mock_app_instance
    )

    # Chamar o método de configuração do ícone
    gui_window._set_app_icon()  # noqa: SLF001
...
def test_gui_logo_in_header(mocker, app):  # noqa: ARG001
    ...
    with patch("PySide6.QtWidgets.QLabel.setPixmap") as mock_set_pixmap:
        ...
        mock_pixmap.scaled.assert_called_with(
...
def test_gui_logo_fallback(mocker, app):  # noqa: ARG001
...
def test_gui_window_flags(gui_window, mocker):  # noqa: ARG001
...
def test_load_otf_fonts(mocker):
    ...
    # Mock para pathlib.Path.exists para cada arquivo de fonte
    mocker.patch(
        "pathlib.Path.exists",
        side_effect=lambda: True,  # Sempre retorna True
    )

    # Mock para QFontDatabase.addApplicationFont para simular sucesso
    mock_add_font = mocker.patch(
        "PySide6.QtGui.QFontDatabase.addApplicationFont", return_value=0
    )

    # Mock para QFontDatabase.applicationFontFamilies para retornar uma lista de famílias
    mock_families = mocker.patch(
        "PySide6.QtGui.QFontDatabase.applicationFontFamilies", return_value=["Inter"]
    )
...
def test_paint_event(gui_window, mocker):
    ...
    mock_path = MagicMock()
    mocker.patch("src.gui.QPainterPath", return_value=mock_path)

    # Mock para QLinearGradient
    mock_gradient = MagicMock()
    mocker.patch(
        "src.gui.QLinearGradient", return_value=mock_gradient
    )

    # Mock para QColor
    mocker.patch("src.gui.QColor")
...
def test_toggle_visibility_with_content(gui_window, mocker):
    ...
    mocker.patch(
        "src.gui.QApplication.screenAt", return_value=mock_screen
    )
    ...
    mocker.patch.object(gui_window, "setWindowState")
    ...
    mocker.patch.object(
        gui_window.original_text_edit, "setText"
    )
    mocker.patch.object(
        gui_window.corrected_text_edit, "setText"
    )
    mocker.patch.object(
        gui_window.original_text_edit, "repaint"
    )
    mocker.patch.object(
        gui_window.corrected_text_edit, "repaint"
    )
...
def test_set_text_content_with_repaint(gui_window, mocker):
    ...
    mock_original_set_text = mocker.patch.object(
        gui_window.original_text_edit, "setText"
    )
    mock_corrected_set_text = mocker.patch.object(
        gui_window.corrected_text_edit, "setText"
    )

    # Mock para QTextEdit.repaint
    mock_original_repaint = mocker.patch.object(
        gui_window.original_text_edit, "repaint"
    )
    mock_corrected_repaint = mocker.patch.object(
        gui_window.corrected_text_edit, "repaint"
    )
    ...
    mock_original_set_text.assert_called_once_with(original_text)
    mock_corrected_set_text.assert_called_once_with(corrected_text)
...
def test_font_loading_error(mocker):
    ...
    # Simular um erro no QFontDatabase.addApplicationFont() com tipo capturado
    mocker.patch("pathlib.Path.exists", return_value=True)
    mocker.patch(
        "PySide6.QtGui.QFontDatabase.addApplicationFont",
        side_effect=OSError("Erro simulado no carregamento de fonte"),
    )
    mock_logger_warning = mocker.patch("src.gui.logger.warning")
    ...
    assert any(
        "Error loading fonts" in call_args[0][0]
        for call_args in mock_logger_warning.call_args_list
    )
...
def test_toggle_visibility_error_handling(gui_window, mocker):
    ...
    # Mock para screenAt que lança uma exceção do tipo capturado (RuntimeError)
    mocker.patch(
        "src.gui.QApplication.screenAt", side_effect=RuntimeError("Erro simulado")
    )
    ...
    assert "Error in toggle_visibility" in call_args
...
def test_font_loading_no_fonts_found(mocker):
    ...
    mocker.patch(
        "PySide6.QtGui.QFontDatabase.applicationFontFamilies", return_value=[]
    )
...
def test_set_processing_state_to_false(gui_window):
    ...
    gui_window._set_processing_state(is_processing=False)  # noqa: SLF001
...
def test_font_loading_application_font_failure(mocker):
    ...
    mocker.patch(
        "PySide6.QtGui.QFontDatabase.addApplicationFont", return_value=-1
    )
    ...
    assert any(
        "Failed to load font" in call_args[0][0]
        and "addApplicationFont returned -1" in call_args[0][0]
        for call_args in mock_logger_warning.call_args_list
    )
...
def test_font_loading_all_failed(mocker):
    ...
    assert any(
        "Font file not found" in call_args[0][0]
        for call_args in mock_logger_warning.call_args_list
    )

    # Verificar que foi emitido o warning final de fallback
    assert any(
        "Could not load any Inter fonts" in call_args[0][0]
        for call_args in mock_logger_warning.call_args_list
    )
