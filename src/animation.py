...
class Particle:
    ...
    def __init__(self, center_x, center_y, size=4):
        self.x = center_x + random.uniform(-50, 50)
        self.y = center_y + random.uniform(-50, 50)
        self.size = random.uniform(1, size)
        self.opacity = random.uniform(0.3, 0.7)
        self.lifetime = random.uniform(0.5, 1.5)  # Seconds
        self.age = 0
        ...
        r = 66 + random.randint(-20, 20)
        g = 133 + random.randint(-20, 20)
        b = 244 + random.randint(-20, 20)
...
class PulseCircle:
    ...
    def __init__(self, size=60, max_size=120, duration=1000):
        """
        ...
        # Color variations
        r = 66 + random.randint(-10, 10)
        g = 133 + random.randint(-10, 10)
        b = 244 + random.randint(-10, 10)
        self.color = QColor(r, g, b)
    ...
    def update(self):
        ...
        if random.random() < 0.3 and len(self.trail_positions) < self.max_trail_length:
            prev_size = self.size
    ...
    def draw(self, painter, center_x, center_y):
        ...
        for i, (trail_size, trail_opacity) in enumerate(self.trail_positions):
            ...
            gradient.setColorAt(1, QColor(trail_color.red(), trail_color.green(), trail_color.blue(), 0))
...
class MagicAnimationWindow(QWidget):
    ...
    def _update_position(self):
        ...
        target_x = max(screen_geometry.left(), min(target_x, screen_geometry.right() - window_width))
        target_y = max(screen_geometry.top(), min(target_y, screen_geometry.bottom() - window_height))
